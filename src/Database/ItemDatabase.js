                ...
                SQLite.echoTest().then(() => {        
                    ...
                    SQLite.openDatabase(database_name, database_version, database_displayname, database_size).then(DB => {
                        ...
                        db.executeSql('SELECT 1 FROM Item LIMIT 1').then(() => {
                            ...
                            db.transaction((tx) => {
                                tx.executeSql('CREATE TABLE IF NOT EXISTS Item (id INTEGER PRIMARY KEY AUTOINCREMENT, nome varchar(30), preco varchar(10))');
                            }).then(() => {
                ...
                this.Conectar().then((db) => {      
                    db.transaction((tx) => {     
                        ...
                        for (let i = 0; i < len; i++) {            
                            ...
                            const {id, nome, preco} = row;
                            lista.push({id, nome, preco});
                        }
                        console.log(lista);          
                        resolve(lista);
                    });
        ...
        return new Promise((resolve) => {    
            this.Conector().then((db) => {      
                db.transaction((tx) => {   
                    ...
                    tx.executeSql('SELECT * FROM Item WHERE id = ?', [id]).then(([tx,results]) => {          
                      console.log(results);         
        ...
        return new Promise((resolve) => {    
            this.Conectar().then((db) => {      
                db.transaction((tx) => {     
                    ...
                    tx.executeSql('INSERT INTO Item (nome,preco) VALUES (?, ?)', [item.nome, item.preco]).then(([tx, results]) => { 
                        resolve(results);        
                    });      
                }).then((result) => {        
                    this.Desconectar(db);      
